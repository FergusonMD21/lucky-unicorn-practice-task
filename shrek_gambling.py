...
def print_prize(bet):
    ...
    global total_spent, total_earnings, tokens
    ...
    # If the token is a Shrek, 4 times the bet is earned
    if token == "Shrek":
    ...
    # If the token is Puss in Boots, or Donkey, it earns hald of the bet
    ...
    # If the token is not Shrek, Puss in Boots, Donkey, it must be a Lord Farquad which earns nothing
    else:  
...
def get_bet():
    # Imports global variables
    global max_spend, total_spent, total_earnings
...
while True:
    # Uses the get_bet function to get the bet amount
    bet = get_bet()

    # Selects a token and prints the prize amount to the user
    print_prize(bet)
